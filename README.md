# Laravel Pstorm Generator

![laravel-pstorm-generator](https://cdn.articlevideorobot.com/hosted/16-12-2022/laravel-pstorm-152f.webp)

## Introduction

[![Software License](https://img.shields.io/badge/license-MIT-green.svg?style=flat-square)](LICENSE.md)
[![Build Status](https://img.shields.io/circleci/project/github/san-kumar/laravel-pstorm.svg?style=flat-square)](https://circleci.com/gh/san-kumar/laravel-pstorm)
[![Coverage Status](https://img.shields.io/badge/coverage-94%25-brightgreen)](https://raw.githack.com/san-kumar/laravel-pstorm/main/build/coverage/index.html)
[![Latest Version on Packagist](https://img.shields.io/packagist/v/san-kumar/laravel-pstorm.svg?style=flat-square)](https://packagist.org/packages/san-kumar/laravel-pstorm)
[![Total Downloads](https://img.shields.io/packagist/dt/san-kumar/laravel-pstorm.svg?style=flat-square)](https://img.shields.io/packagist/dt/san-kumar/laravel-pstorm.svg?style=flat-square)



> ## Please see the [**full documentation here**](https://san-kumar.github.io/laravel-pstorm/).


This package adds a **pstorm:generate** command to Artisan in
your Laravel project. With this command you can generate
Pstorm (Create, Read, Update, Delete) for your db tables
instantly!

The generated Pstorm includes a Controller, Model, Policy,
routes, validations, and 4 view files (index, create, edit,
show). It also supports relations between tables, so you can
generate nested CRUDs, for example: generating Pstorm for
...
Laravel-pstorm can generate nested CRUDs. For example, you can
generate Pstorm for customers > tickets > replies by simply
running:

```bash
$ php artisan pstorm:generate customers.tickets.replies
```
...
```bash
$ php artisan pstorm:generate customers.tickets.replies -i cards
```
...
```bash
$ php artisan pstorm:generate customers.tickets.replies -i chat
```
...
Laravel Pstorm generator can automatically generate the FORM
...
The aim of the Pstorm generator is to save you time by
generating common boilerplate Pstorm code for any database
table instantly.
...
```bash
$ composer require san-kumar/laravel-pstorm --dev
```

## Generate files

After installing the package you should see a
new `pstorm:generate` command in your Artisan commands list.

To generate the Controller, Model, Policy, routes, etc use
the following command:

```bash
# Create Pstorm for authors table
$ php artisan pstorm:generate authors

# Create Pstorm for the authors &gt; posts table
$ php artisan pstorm:generate authors.posts

# Create Pstorm for the authors &gt; posts &gt; comments table  
$ php artisan pstorm:generate authors.posts.comments
```

## Including the generated routes

By default, the generated routes are placed in
the `./routes/pstorm/` directory. To include the generated
...
the `./routes/pstorm/` directory to your `routes/web.php`
file. This way you make any changes to the generated route
code as per your needs.

## Location of the generated files

The `pstorm:generate` command will generate the following
...
```bash
# Create Pstorm for authors table using Tailwind CSS
$ php artisan pstorm:generate authors --css=tailwind

# Shortcut for Tailwind CSS
$ php artisan pstorm:generate authors -t
```

```bash
# Create Pstorm for authors table using layout 'layouts/admin.blade.php'
$ php artisan pstorm:generate authors --layout=admin
$ php artisan pstorm:generate authors --section=mysection
```

### Prefix

You can add a route prefix for the generated Pstorm by passing
a `--prefix` option.

```bash
# Create Pstorm for authors table using Tailwind CSS
$ php artisan pstorm:generate authors --prefix=admin
```
...
```bash
# Create Pstorm for authors table using cards template
$ php artisan pstorm:generate authors -i cards

# Create Pstorm for authors table using chat template
$ php artisan pstorm:generate authors -i chat
```

Please see
the [index page here](https://san-kumar.github.io/laravel-pstorm/guide/index.md#screenshots) for
screenshots.
...
In addition to generating simple Pstorm for a single table,
you can also generate Pstorm for tables related to each other.
...
To generate Pstorm for these tables you can use the dot
...
# Create Pstorm for posts table
$ php artisan pstorm:generate authors.posts

# Create Pstorm for comments table
$ php artisan pstorm:generate authors.posts.comments
```

This will generate Pstorm files for the `authors` table, so
...
On the other hand, if you want to generate Pstorm for the
`posts` table without nesting it under the `authors` route,
you can do this:

```bash
# Create Pstorm for posts table
$ php artisan pstorm:generate posts

# Create Pstorm for comments table
$ php artisan pstorm:generate posts.comments
```
...
Laravel Pstorm generator uses the default Laravel
authentication
package [laravel/ui](https://laravel.com/docs/7.x/authentication#included-routing)
for handling user authentication. The Pstorm generator will
...
If you generate Pstorm for any table that has a `user_id`
field, then the generated Pstorm will automatically generate a
...
If you generate Pstorm for a table that has a `user_id` field
in the parent tables (i.e. `authors.posts.comments` has a
`user_id` field in the `authors` table), then the generated
Pstorm will automatically generate a Policy file for
...
Laravel Pstorm generator handles tables with soft deletes
...
At the heart of Laravel Pstorm generator are the templates
files that are used to generate the Pstorm files.

The templates files included with this package are stored in
the `./vendor/san-kumar/laravel-pstorm/src/template`
directory.
...
```bash
# Copy the template files into a new directory
$ php artisan pstorm:template newname -d etc/templates
```
...
directory in the `config/pstorm.php` file.
...
the `--template` or `-t` option while generating the Pstorm.

```bash
# Generate the Pstorm using the new template
$ php artisan pstorm:generate Post --template etc/templates/newname
```
...
[GenerateCrud.php](https://github.com/san-kumar/laravel-pstorm/blob/main/src/Commands/CrudGenerate.php#L49)
...
Laravel Pstorm generator doesn't seem to work with
...
generated Pstorm uses Laravel's `Policy` classes for handling
authorization. But you may prefer to use something else
like `spatie/laravel-permission`, which can be done but
requires you
to [create your own templates](https://san-kumar.github.io/laravel-pstorm/guide/customization).
...
# Removing the generated Pstorm files

## Remove files

Laravel Pstorm Generator allows you to remove the generated
files using the `artisan pstorm:remove` command.

```bash
$ php artisan pstorm:remove customers.tickets.replies
```

This will remove all the files generated by
the `pstorm:generate` for this table.

You can also remove multiple tables at once by passing
multiple table names separated by a space.

```bash
$ php artisan pstorm:remove customers customers.tickets customers.tickets.replies
```
...
```bash
$ php artisan pstorm:remove customers.tickets.replies --backup=backup.zip
```
...
Automatically generate unit tests for all generated Pstorm.
